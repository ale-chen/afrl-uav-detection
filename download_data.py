...
def download_wav_files(credentials_file, download_string, base_directory, output_directory):
    ...
    try:
        # Search for the series folder first
        query = f"name='{series}' and mimeType='application/vnd.google-apps.folder'"
        print(f"Searching for series folder: {query}")
        results = service.files().list(q=query, fields="files(id)").execute()
        series_folders = results.get("files", [])
        
        if not series_folders:
            print(f"Series folder '{series}' not found.")
        else:
            series_folder_id = series_folders[0]["id"]
            print(f"Found series folder: {series_folder_id}")
            
            # Search for the p_level folder within the series folder
            query = f"'{series_folder_id}' in parents and name='{p_level}' and mimeType='application/vnd.google-apps.folder'"
            print(f"Searching for p_level folder: {query}")
            results = service.files().list(q=query, fields="files(id)").execute()
